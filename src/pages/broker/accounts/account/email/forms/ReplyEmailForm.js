...
export default function ReplyEmailForm({
  data,
  subject = '',
  to = [],
  cc = [],
  ...
  const [TOs, setTO] = useState({
    emails: to ? to.map((email) => email.email) : [],
  })
  const [CCs, setCC] = useState({
    emails: cc ? cc.map((email) => email.email) : [],
  })
